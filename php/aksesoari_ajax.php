<?php
    ...
    if(isset($_POST['korisnicko_ime_aksesoari']) && isset($_POST['komentar_aksesoari']) && isset($_POST['ocena_aksesoari'])){
    $values = array($_POST["korisnicko_ime_aksesoari"],$_POST["komentar_aksesoari"],$_POST["ocena_aksesoari"]);
    $crud=new Database();
    $crud->insert("aksesoari",$values);
    }

    if(isset($_POST['delete_id'])){
        $id=$_POST['delete_id'];
        $crud=new Database();
        $crud->delete("aksesoari",$id);
    }
    ...
    if(isset($_POST['komentar']) && isset($_POST['ocena'])){
        ...
        $crud->update("aksesoari",$values);
    }
    
    if(isset($_POST['search_post_btn'])){
        $ime=$_POST['ime'];
        $crud=new Database();
        $query_run=$crud->select("aksesoari","id,korisnicko_ime,komentar,ocena","korisnicko_ime='$ime'");
   ...
   if(isset($_POST['btn'])){
    $crud=new Database();
    $query_run=$crud->select("aksesoari","id,korisnicko_ime,komentar,ocena",null,"ocena DESC");
    ?>
    <thead>
    <tr>
      <th scope="col"> <h3>Ime</h3> </th>
