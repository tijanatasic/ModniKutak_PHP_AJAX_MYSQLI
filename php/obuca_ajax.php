<?php
    ...
    if(isset($_POST['korisnicko_ime_obuca']) && isset($_POST['komentar_obuca']) && isset($_POST['ocena_obuca'])){
    $values = array($_POST["korisnicko_ime_obuca"],$_POST["komentar_obuca"],$_POST["ocena_obuca"]);
    $crud=new Database();
    $crud->insert("obuca",$values);
    }
    ...
    if(isset($_POST['komentar']) && isset($_POST['ocena'])){
        ...
        $crud->update("obuca",$values);
    }

    if(isset($_POST['delete_id'])){
        $id=$_POST['delete_id'];
        $crud=new Database();
        $crud->delete("obuca",$id);
    }

    if(isset($_POST['search_post_btn'])){
        $ime=$_POST['ime'];
        $crud=new Database();
        $query_run=$crud->select("obuca","id,korisnicko_ime,komentar,ocena","korisnicko_ime='$ime'");
   ...
   if(isset($_POST['btn'])){
    $crud=new Database();
    $query_run=$crud->select("obuca","id,korisnicko_ime,komentar,ocena",null,"ocena DESC");
    ?>
    <thead>
    <tr>
      <th scope="col"> <h3>Ime</h3> </th>
